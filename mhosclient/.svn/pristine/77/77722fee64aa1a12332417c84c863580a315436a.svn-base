		...
		<div class="right-cnt-wrap f-fl">
			...
			<s:form method="post" id="form_refund" cssClass="introduction-fm">
					<ul>
						...
						<li><span class="introduction-label f-fl" style="width:120px;text-align:left"> <span
								class="color-red">*</span>退款金额(元): </span> 
								<s:textfield
								id="amount" name="amount"
								Class="fm-text f-fl" style="width:550px;" readonly="true"/>
							<div class="f-clear"></div></li>
					...
					<s:hidden name="createDate" id="createDate"/>
					<s:hidden name="accountCode" id="accountCode"/>
					<s:hidden name="status" id="status"/>
					<s:hidden name="billStatus" id="billStatus"/>
			</s:form>
			</div>
		</div>
		...
		$.post("unpaid/refundAction_refund",{
			...
			if (obj.code==0){
				alert("退款成功");
				 ...
				 if(flag.equles('5')){
				var creatDate=$("#createDate").val();
				var status=$("#status").val();
				var billStatus=$("#billStatus").val();
				var accountCode=$("#accountCode").val();
				   window.location.href="unpaid/checkBillAction_findDetails?creatDate="+creatDate+"&status="+status+"&billStatus="+billStatus+"&accountCode="+accountCode;
				 }else{
				   window.location.href="refund/refund_main.jsp";
				 }
			}else if(obj.code==7){
				alert("密码错误");
			}else {
