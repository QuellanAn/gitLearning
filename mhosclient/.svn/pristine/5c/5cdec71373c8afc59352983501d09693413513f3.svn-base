...
var datalist_ctrl = {
	...
	loaddatalist1:function(pageNo,payType,startdate,enddate,status,pattern,districtId,
			out_trade_no,hisTransId,orderCode,body,cardNo,patientName,payScene){
		var cnt = $('#div_cnt_table');
		/*
...
function toSelect(orderId){		
	window.location.href="unpaid/payOrderAction_findPayOrRechargeDetails?orderId="+orderId+"&redirectType=0";
}
...
function loadPayScene(){
	...
	$.get(url , function(data, status){
		...
		for (var i in jarr){
			var o;
			if(jarr[i].sCode != 4 && jarr[i].sCode != 5){
				o = $('<option></option>').val(jarr[i].SCode).html(jarr[i].SName);
			}
