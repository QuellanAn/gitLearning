...
public class ServiceEvaluationHosDAOImpl extends JdbcDaoSupport implements  ServiceEvaluationHosDAO{
	...
	public List<ServiceEvaluationHosPO> findListByParas(String doctorName,String departmentCode,
			...
			int length,String cardNumber,String patientname, String departmentName, String evaSource) {
		try {
		JdbcTemplate jt = this.getJdbcTemplate();
		SimpleDynamicSQL.SelectSQL sql = new SimpleDynamicSQL.SelectSQL();
		sql.setBaseSQL(
				"select a.reg_source_name as sourceName,a.reg_source AS regSource,a.card_number AS cardNumber,a.patient_name AS patientName,a.department_name AS departmentName,a.doctor_name AS doctorName,a.doctor_date AS lookDate,s.* " +
				"FROM service_evaluation_hos s LEFT JOIN app_reg_orders a ON s.app_reg_order_id=a.app_reg_order_id ",
				true);
		...
		if(startDate!=null){
			...
			sql.addPara("s.createDate", ">=", startDate1);
		}
		if(endDate!=null){
			Date date = new Date(endDate.getTime());
			String endDate1 = DateUtil.date2Str(date,"yyyy-MM-dd")+" 59:23:23";
			sql.addPara("s.createDate", "<=", endDate1);
		}
		if(StringUtils.isNotBlank(cardNumber)){
			sql.addPara("a.card_number", "like", "%"+cardNumber+"%");
		}
		if(StringUtils.isNotBlank(evaSource)){
			sql.addPara("a.reg_source", "=", evaSource);
		}
	...
	public ServiceEvaluationHosPO findServiceEvaluationHosById(
		...
		sql.append("select a.reg_source_name as sourceName,a.reg_source AS regSource,a.card_number AS cardNumber,a.patient_name AS patientName,a.department_name AS departmentName,a.doctor_name AS doctorName,a.doctor_date AS lookDate,s.* " +
				"FROM service_evaluation_hos s LEFT JOIN app_reg_orders a ON s.app_reg_order_id=a.app_reg_order_id ");
