			...
			<div class="tab-cnt">
				<div class="f-fl left-list-wrap">
					<div class="left-list-hd">
						<span class="f-fl">挂号总量(<span id="totalcount">0</span>)
						</span>
					</div>
					<div class="left-list-hd">
...
<script type="text/javascript">
	var colors, name = "科室挂号统计", categories = [], data = [], setChart, chart;
...
// 			},
	...
	function addNewSeries() {
		...
		$.post("statistics/getRegistrationSeries", {start : start,end : end, type:"1"}, function(result) {
	...
	function init() {
		...
		$("#enddate").val(end);
		chart.showLoading("加载中...");
		$.post("statistics/getRegistrationSeries", {start : start+" 00:00:00",end : end+" 23:59:59",type:"1"}, function(result) {
			result=result.replace("\n", "\\n").replace("\r", "\\r");
			result = eval(result);
			var count = result.pop();
			$("#totalcount").html(count);
