...
public class AppRegOrdersDAOImpl extends JdbcDaoSupport implements
	...
	public List<AppRegOrdersPO> findAppRegOrdersByParas(String orderid,
			...
			String doctorName, String departmentCode, String paid, String departmentName, String regSource) {
			List<AppRegOrdersPO> lst = null;
		try {
			JdbcTemplate jt = this.getJdbcTemplate();
			String sql = "select a.*, u.user_name from users u, app_reg_orders a where a.user_id = u.user_id ";
			...
			if (StringUtils.isNotBlank(regSource)) {
				sb.append(" AND a.`reg_source` = '"+regSource+"'");
			}
	...
	public int countAppRegOrdersByParas(String orderid, String patientname,
		...
		try {
			...
			if (StringUtils.isNotBlank(regSource)) {
				sql.addPara("reg_source", "=", regSource);
			}
	...
	public List<AppRegOrdersPO> findAppRegOrdersPOByAppRegOrderId(
		...
		// TODO Auto-generated method stub
		try {
			String sql = "select a.* from app_reg_orders a where a.app_reg_order_id = '"+appRegOrderId+"'";
