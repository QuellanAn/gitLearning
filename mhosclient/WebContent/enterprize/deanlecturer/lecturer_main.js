$(document).ready(function() {
	datalist_ctrl.loaddatalist("enterprize/lecturer_findAll?pageNo="+$("#hid_pageNo").val());
	
	$('#btn_add_pub').click(function(){
		window.location.href = 'enterprize/deanlecturer/lecturer_add.jsp';
	});
	
	$("#act_query").click(function(e){
		var paras = $("#form_query").serialize();
		var pageNo=$("#hid_pageNo").val();
		var url = "enterprize/lecturer_findAll?pageNo="+pageNo+"&" + paras;
...
function deletehosinfo(id){
	if (confirm("你确认要删除该条记录吗？")){
		$.post("enterprize/lecturer_delete",
				{id:id},
...
function updatehosinfo(id) {
	document.location.href="enterprize/lecturer_toUpdate?id="+id;
}
...
var datalist_ctrl = {
		loaddatalist:function(url){
			...
			loaddatalist1:function(pageNo){
				var cnt = $('#div_cnt_table');
				$.post('enterprize/lecturer_findAll',{
					pageNo:pageNo
						},
